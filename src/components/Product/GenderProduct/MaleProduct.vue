  ...
  >
    ...
    <section class="bg-white py-8">
      <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        <div
          class="w-full md:w-2/4 xl:w-1/4 p-16 flex flex-col"
          v-for="male in filteredMaleProduct"
          :key="male.id"
          :male="male"
        >
          <img
            class="hover:grow hover:shadow-lg w-72 h-72 object-cover lg:self-center sm:self-center mobile"
            :src="`https://walkincloset.ddns.net/backend/Images/Get/${male.productId}.jpg`"
            @click="emitShow(male.productId)"
          />
          <div class="space-y-2">
            ...
            <p class="text-md Barlow font-bold">
              {{ male.brandId.brandName }}
            </p>
            <div class="space-x-2">
              <color-block
                width="w-6"
                height="h-6"
                v-for="malecolor in male.productColors"
                :key="malecolor.colorId"
                :codeColor="malecolor.codeColor"
                class="border border-black"
              />
            </div>
            <p class="text-gray-900 text-2xl Barlow-Medium">
              {{ male.productPrice }} THB
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      searchMale: "",
      urlMale:"https://walkincloset.ddns.net/backend/Products/GetProducts/Gender/Male",
      allMaleProduct: [],
    };
  },
  props: ["product"],
  ...
  computed: {
    filteredMaleProduct() {
      return this.allMaleProduct.filter((male) =>
        male.productName.toLowerCase().includes(this.searchMale.toLowerCase()) ||
         male.brandId.brandName.toLowerCase().includes(this.searchMale.toLowerCase()) ||
         (male.productPrice + '').includes(this.searchMale)
      );
    },
  },
};
