  ...
  >
    ...
    <section class="bg-white py-8">
      <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        <div
          class="w-full md:w-2/4 xl:w-1/4 p-16 flex flex-col"
          v-for="female in filteredFemaleProduct"
          :key="female.id"
          :male="female"
        >
          <img
          ...
          <div class="space-y-2">
            ...
            <p class="text-md Barlow font-bold">
              {{ female.brandId.brandName }}
            </p>
            <div class="space-x-2">
              <color-block
                width="w-6"
                height="h-6"
                v-for="femalecolor in female.productColors"
                :key="femalecolor.colorId"
                :codeColor="femalecolor.codeColor"
                class="border border-black"
              />
            </div>
            <p class="text-gray-900 text-2xl Barlow-Medium">
              {{ female.productPrice }} THB
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      searchfemale: "",
      urlFemale:"https://walkincloset.ddns.net/backend/Products/GetProducts/Gender/Female",
      allFemaleProduct: [],
    };
  },
  props: ["product"],
