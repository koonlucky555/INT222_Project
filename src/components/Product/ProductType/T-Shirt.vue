  ...
  >
    ...
    <section class="bg-white py-8">
      <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        <div
          class="w-full md:w-2/4 xl:w-1/4 p-16 flex flex-col"
          v-for="tshirt in filteredTShirtProduct"
          :key="tshirt.id"
          :tshirt="tshirt"
        >
          <img
            class="hover:grow hover:shadow-lg w-72 h-72 object-cover lg:self-center sm:self-center mobile"
            :src="`https://walkincloset.ddns.net/backend/Images/Get/${tshirt.productId}.jpg`"
            @click="emitShow(tshirt.productId)"
          />
          <div class="space-y-2">
            ...
            <p class="text-md Barlow font-bold">
              {{ tshirt.brandId.brandName }}
            </p>
            <div class="space-x-2">
              <color-block
                width="w-6"
                height="h-6"
                v-for="tshirtcolor in tshirt.productColors"
                :key="tshirtcolor.colorId"
                :codeColor="tshirtcolor.codeColor"
                class="border border-black"
              />
            </div>
            <p class="text-gray-900 text-2xl Barlow-Medium">
              {{ tshirt.productPrice }} THB
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      searchTShirt: "",
      urlTShirt:"https://walkincloset.ddns.net/backend/Products/GetProducts/Type/T-Shirt",
      allTShirtProduct: [],
    };
  },
  props: ["product"],
  ...
  computed: {
    filteredTShirtProduct() {
      return this.allTShirtProduct.filter((Tshirt) =>
        Tshirt.productName.toLowerCase().includes(this.searchTShirt.toLowerCase()) || 
         Tshirt.brandId.brandName.toLowerCase().includes(this.searchTShirt.toLowerCase()) ||
         (Tshirt.productPrice + '').includes(this.searchTShirt)
      );
    },
  },
};
