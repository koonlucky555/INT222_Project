  ...
  >
    ...
    <section class="bg-white py-8">
      <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        <div
          class="w-full md:w-2/4 xl:w-1/4 p-16 flex flex-col"
          v-for="sweater in filteredSweaterProduct"
          :key="sweater.id"
          :sweater="sweater"
        >
          <img
          ...
          <div class="space-y-2">
            ...
            <p class="text-md Barlow font-bold">
              {{ sweater.brandId.brandName }}
            </p>
            <div class="space-x-2">
              <color-block
                width="w-6"
                height="h-6"
                v-for="sweatercolor in sweater.productColors"
                :key="sweatercolor.colorId"
                :codeColor="sweatercolor.codeColor"
                class="border border-black"
              />
            </div>
            <p class="text-gray-900 text-2xl Barlow-Medium">
              {{ sweater.productPrice }} THB
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      searchSweater: "",
      urlSweater:"https://walkincloset.ddns.net/backend/Products/GetProducts/Type/Sweatshirt",
      allSweaterProduct: [],
    };
  },
  props: ["product"],
