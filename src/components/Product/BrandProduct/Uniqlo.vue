  ...
  >
    ...
    <section class="bg-white py-8">
      <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
        ...
        <div
          class="w-full md:w-2/4 xl:w-1/4 p-16 flex flex-col"
          v-for="unq in filteredUniqloProduct"
          :key="unq.id"
          :unq="unq"
        >
          <img
            class="hover:grow hover:shadow-lg w-72 h-72 object-cover"
            :src="`https://walkincloset.ddns.net/backend/Images/Get/${unq.productId}.jpg`"
            @click="emitShow(unq.productId)"
          />
          <div class="space-y-2">
            ...
            <p class="text-md Barlow font-bold">
              {{ unq.brandId.brandName }}
            </p>
            <div class="space-x-2">
              <color-block
                width="w-6"
                height="h-6"
                v-for="unqcolor in unq.productColors"
                :key="unqcolor.colorId"
                :codeColor="unqcolor.codeColor"
                class="border border-black"
              />
            </div>
            <p class="text-gray-900 text-2xl Barlow-Medium">
              {{ unq.productPrice }} THB
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      searchUniqlo: "",
      urlUniqlo:"https://walkincloset.ddns.net/backend/Products/GetProducts/Brands/unq",
      allUniqloProduct: [],
    };
  },
  props: ["product"],
